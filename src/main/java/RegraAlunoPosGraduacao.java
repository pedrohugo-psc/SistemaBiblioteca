...
public class RegraAlunoPosGraduacao implements IRegra{
    ...
    public boolean buscaEmprestimoDataUsuario(LocalDate dataAtual, int codigoUsuario){
        ...
        for (IEmprestimo emprestimo : emprestimos) {
            if ((emprestimo.getIdUsuario() == codigoUsuario) && ((ChronoUnit.DAYS.between(emprestimo.getData(),dataAtual)) > 5)) {
                resultado = false;
            }
        }
        return resultado;
    ...
    public LocalDate DataPrevistaDevolucao(LocalDate dataEmprestimo){
        LocalDate dataPrevistaDevolucao = dataEmprestimo.plusDays(5);
